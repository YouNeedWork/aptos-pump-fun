module pump::pump {
    ...
    use aptos_std::type_info::{type_of, type_name};
    ...
    */
    ...
    struct Pool<phantom CoinType> has key,store {
     ...
     fun calculate_add_liquidity_cost(apt_reserves: u256, token_reserves: u256, virtual_token_reserves: u256): u256 {
    ...
    fun calculate_remove_liquidity_return(token_reserves: u256, apt_reserves: u256, liquidity_removed: u256) : u256 {
        token_reserves - ((token_reserves * liquidity_removed) / (liquidity_removed + apt_reserves))
    }

    fun calculate_token_amount_received(apt_reserves: u256, token_reserves: u256, liquidity_removed: u256):u256 {
        token_reserves - ((apt_reserves * token_reserves) / (apt_reserves + liquidity_removed))
    }
    ...
    public fun deploy<CoinType>(
        ...
        let deploy_coin = coin::mint<CoinType>(config.initial_virtual_token_reserves,&mintCap);
    ...
    public fun deploy_and_buy<CoinType>(
        caller: &signer,
    ) {
    ...
    public fun buy<CoinType>(
        ...
        let token_reserve_difference = pool.virtual_token_reserves - coin::value(&pool.remain_token_reserves);
        let token_amount = math64::min(out_amount, token_reserve_difference);

        let liquidity_cost = calculate_add_liquidity_cost(
            (pool.virtual_apt_reserves as u256), (pool.virtual_token_reserves as u256), (token_amount as u256)) + 1;
        /*
        ...
        */
        ...
        let (received_token, remaining_apt) = swap<CoinType>(pool,coin::zero<CoinType>(),apt_coin, token_amount, apt_amount - (liquidity_cost as u64));
        pool.virtual_token_reserves = pool.virtual_token_reserves - coin::value(&received_token);
